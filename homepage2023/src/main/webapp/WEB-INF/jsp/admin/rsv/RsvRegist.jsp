				...
				<div class="btn-cont ar">
					<c:choose>
						<c:when test="${not empty searchVO.resveId}">
							...
							<a href="${uptUrl}" class="btn" id="btn-reg">수정</a>
							
							<c:url var="delUrl" value="/admin/rsv/rsvDelete.do${_BASE_PARAM}">
								<c:param name="resveId" value="${result.resveId}" />
							</c:url>
							<a href="${delUrl}" class="btn" id="btn-del"><i class="ico-del"></i> 삭제</a>
						</c:when>
						<c:otherwise>
							<a href="/admin/rsv/rsvInsert.do" class="btn spot" id="btn-reg">등록</a>
						</c:otherwise>
					</c:choose>
